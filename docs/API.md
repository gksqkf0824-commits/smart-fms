...
## 1. AI 추론 서버 (FastAPI) — 담당: 왕석빈(구축)·권소윤(연동)
...
| 필드 | 타입 | 설명 |
| --- | --- | --- |
| image | file | (마스킹된) 실내 사진 1장. **JPG로 통일되어 들어옴** (백엔드가 변환) |
...
> **임계치 근거:** 면적 비율로 오염을 정량화·등급화하는 *방법론*은 식물병·산업 결함 진단 등에서 확립된 접근을 따름.
> 다만 5%·2%·3개 같은 *임계치 수치*는 학술값이 아니라 **운영 정책 파라미터**(운영사가 비용·고객경험 고려해 조정).
> 따라서 하드코딩하지 않고 설정값으로 분리. 초기값은 실제 세차비가 발생하는 "명백한 오염" 수준을 참고해 보수적으로 설정, 운영 데이터로 보정 전제.
...
**내부 동작:** 사진 수신 → 형식 검증·JPG 변환 → 마스킹 → S3 저장 → `POST /predict` 호출 → 등급 판정 → 배차차단·세차·패널티·알림 → DB 기록
...
| image | file | 실내 사진 **원본** (마스킹·형식 변환은 서버가 함) |

#### 입력 이미지 형식 처리

- **허용 형식:** JPG, PNG, HEIC (아이폰·안드로이드 촬영 모두 대응)
- **처리 위치:** 백엔드(Spring Boot) 수신 단계에서 검증·변환
  - 허용 형식 → **JPG로 통일 변환** 후 S3 저장·AI 전달 (S3 `contentType`도 실제 형식에 맞게 설정)
  - 미지원 형식(gif 등)·비이미지·용량 초과 → `400` 에러 (`"지원하지 않는 이미지 형식"`)
- **AI 서버(`/predict`)는 JPG를 받는다고 가정** — 형식 걱정은 백엔드가 처리.
...
- **백엔드팀:** raw 값 받아 2번 판정 로직으로 `grade` 계산 → 조치 실행. 기존 `classes[]` 파싱을 `trash_count`/`trash_large`/`occupy_detected`/`roi_pollution_ratio`로 **수정 필요.** 입력 이미지 형식 검증·JPG 변환도 수신 단계에서 처리.
...
| 2026-08-29 | 2모델(Detection+Segmentation) 구조 반영. `classes[]` → `trash_count`/`trash_large`/`occupy_detected` + `roi_pollution_ratio`(spill). 필드명·0~1 값 형식 유지. `occupy`/`user_alert`, 입력 이미지 형식 처리, 임계치 근거 추가 | 권소윤 |
| 2026-09-06 | AI 서버 담당(왕석빈) 확정, 입력 이미지 형식 처리·임계치 근거 추가 | 권소윤 |
